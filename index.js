...
autocompleteInput.addEventListener("keyup",(event)=>{
    ...
    resultProduct=productStore.filter(product=>{
        return product.title.toLowerCase().includes(searchInput);
    })

    const hasProductToShow=resultProduct.length>0;
    ...
    if(hasProductToShow){
        for(let index=0; index<resultProduct.length; index++){
            ...
            productName.append(resultProduct[index].title);
...
const selectProduct=(key)=>{
    if(key === "ArrowDown"){
        ...
        selectIndex+=1;
        const productItemSelectContainer=productSelection(selectIndex);
...
const productSelection=(index)=>{
    const productId=resultProduct[index].id.toString();
    const productItemSelectContainer=document.getElementById(productId);
    console.log(productItemSelectContainer);
    productItemSelectContainer.style.backgroundColor="maroon";
    productItemSelectContainer.firstChild.style.color="white";
    return productItemSelectContainer;
}
const productDeselection=()=>{
    const productToDeselect=document.getElementsByClassName("selected")[0];
    console.log(productToDeselect);
